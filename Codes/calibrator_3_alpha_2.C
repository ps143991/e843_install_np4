...
// --------------------------------------------------------
double main_peak_Pu=0;
double main_peak_Am=0;
double main_peak_Cm=0;

double coeff = 0.0082971; // MeV/channel
double offset = -67.9685; // MeV

double calib_Pu = 0;
double calib_Am = 0;
double calib_Cm = 0;

nptool::EnergyLoss LightAl = nptool::EnergyLoss(Path+light + "_Al.G4table", "G4Table", 100);
                    ...
                    double sigmaPu, double sigmaAm, double sigmaCm)
{
  ...
  main_peak_Pu = Pu->GetParameter(1);
  main_peak_Am = Am->GetParameter(1);
  main_peak_Cm = Cm->GetParameter(1);
...
void calibrator_3_alpha_2(){
  ...
  calib_Pu = main_peak_Pu * coeff + offset;
  calib_Am = main_peak_Am * coeff + offset;
  calib_Cm = main_peak_Cm * coeff + offset;

  cout<<"Calibrated Pu peak: "<< calib_Pu << " MeV"<<endl;
  cout<<"Calibrated Am peak: "<< calib_Am << " MeV"<<endl;
  cout<<"Calibrated Cm peak: "<< calib_Cm << " MeV"<<endl;
