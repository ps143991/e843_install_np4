...
/*     TF1* bglandau = new TF1("bglandau", [](double* x, double* par) {
    ...
    peak10gfit->Draw("same");
            ...
            TF1* peak11gfit = new TF1("peak11fit", gaus_plus_pol4, 1.3, 3.3, 8);
...
using namespace std;
void response_fitPS_v10_most_successful_so_far()
{
    ...
    Int_t totalparams = 5 + (3*6) + (3*5); // 5 for pol4, 6 simulated gaus, 2 new gaus
...
for (int i = 0; i < 5; ++i) {
    ...
    for(int li = 5; li < 23; ++li) {
        ...
        whole->SetParameter(29, 10); whole->SetParameter(30, 3.7); whole->SetParameter(31, 0.25);
        whole->SetParLimits(29, 1,200); whole->SetParLimits(30, 3.7-0.2, 3.7+0.2); whole->SetParLimits(31, 0.15,0.25);

        whole->SetParameter(32, 10); whole->SetParameter(33, 4.8); whole->SetParameter(34, 0.25);
        whole->SetParLimits(32, 1,200); whole->SetParLimits(33, 4.8-0.2, 4.8+0.2); whole->SetParLimits(34, 0.15,0.25);

        whole->SetParameter(35, 10); whole->SetParameter(36, 2.3); whole->SetParameter(37, 0.25);
        whole->SetParLimits(35, 1,200); whole->SetParLimits(36, 2.28-0.10, 2.28+0.10); whole->SetParLimits(37, 0.15,0.25);
    ...
    for(int ill = 0; ill < totalparams; ++ill) {
        ...
        whole_region->SetParLimits(5, 1, 200);   whole_region->SetParLimits(6,  fit_parameters[6]  -0.005,fit_parameters[6]  + 0.005);  whole_region->SetParLimits(7, fit_parameters[7] -0.02, fit_parameters[7] + 0.02);
