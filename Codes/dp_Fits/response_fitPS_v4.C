...
void response_fitPS_v4()
{
    ...
    TH1F* all_ex_inst2 = (TH1F*)datafile->Get("ex");
    ...
    can1 = new TCanvas("can1", "carbon ex", 800, 600);

    all_ex->SetLineColor(kRed);
    all_ex->SetLineWidth(2);
    ...
    TF1* fitpol3_data = new TF1("fitpol3_data", "pol3", -5, 17);
    fitpol3_data->SetLineColor(kRed);
    fitpol3_data->SetLineWidth(3);
    fitpol3_data->SetParameters(fitpol3->GetParameters());
                    ...
                    TF1 *fCarbonScaled = new TF1(
                    "fCarbonScaled",
                    "[0]*( [1] + [2]*x + [3]*x*x + [4]*x*x*x )",
                    -5, 17
                    );
                    ...
                    can2= new TCanvas("can2", "data ex", 800, 600);
                    all_ex->Rebin(8);
                    all_ex->Draw("hist same");              // draw as histogram (no errors)
                    fCarbonScaled->Draw("same"); // draw the scaled carbon fit


                    
    TCanvas* call= new TCanvas("call", "Response Histograms", 1910, 1070);
    call->Divide(3, 2);
...
/* 
    ...
    TF1* whole = new TF1("whole", allgaus_plus_left_skewed_landau, -1, 15, 21);


    for(int ll = 0; ll<18; ll++){
    whole->SetParameter(ll,fit_parameters[ll]);}
    whole->SetParameter(18, 145);        // background amplitude
    whole->SetParameters(19, 7.96);        // MPV
    whole->SetParameters(20, 1.63);           // sigma

    cout<< "Fit parameters: " << endl;
    for (int i = 0; i < 21; ++i) {
        cout << "Parameter " << i << ": " << fit_parameters[i] << endl;
    }
...
/*     TF1* bcf = new TF1("bcf", left_skewed_landau, -5, 15, 3);
    cout<< "BCF parameters: " << endl;
    for (int i = 0; i < 3; ++i) {
        cout << "Parameter " << i << ": " << whole->GetParameter(18 + i) << endl;
    }
    bcf->FixParameter(0,whole->GetParameter(18));
    bcf->FixParameter(1,whole->GetParameter(19));
    bcf->FixParameter(2,whole->GetParameter(20));
