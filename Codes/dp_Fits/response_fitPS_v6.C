...
/* 
    ...
    TF1* whole = new TF1("whole", allgaus_plus_pol4, -1, 5.2, 26);
    
    for(int li = 0; li < 18; ++li) {
        whole->SetParameter(li, fit_parameters[li]);}
        
        //MORE BREATHING ROOM FOR THE 2nd peak*************************
        whole->SetParLimits(0, 1,70);   whole->SetParLimits(1,  fit_parameters[1] -0.2, fit_parameters[1]  +0.2);  whole->SetParLimits(2, 0.2, 0.3);
        whole->SetParLimits(3, 1,70);   whole->SetParLimits(4,  fit_parameters[4] -0.4, fit_parameters[4]  +0.2);  whole->SetParLimits(5, 0.2, 0.3);
        whole->SetParLimits(6, 1,70);   whole->SetParLimits(7,  fit_parameters[7] -0.2, fit_parameters[7]  +0.2);  whole->SetParLimits(8, 0.2, 0.3);
        whole->SetParLimits(9, 1,70);   whole->SetParLimits(10, fit_parameters[10]-0.2, fit_parameters[10] +0.2);  whole->SetParLimits(11,0.2, 0.3);
        whole->SetParLimits(12, 1,70);  whole->SetParLimits(13, fit_parameters[13]-0.2, fit_parameters[13] +0.2);  whole->SetParLimits(14,0.2, 0.3);
        whole->SetParLimits(15, 1,70);  whole->SetParLimits(16, fit_parameters[16]-0.2, fit_parameters[16] +0.2);  whole->SetParLimits(17,0.2, 0.3);
    ...
    // Save fit parameters and integrals to CSV
	 ...
	 TLegend* legend = new TLegend(0.75, 0.15, 0.90, 0.55);
        ...
        //   legend->AddEntry((TObject*)nullptr, Form("ThetaLab: %d °", (angmin + angmax)/2), "");
          ...
          legend->AddEntry(peak7gfit, Form("new peak @ %.0f", fit_parameters[24]*1000), "l");
