...
void confirming_DC_np3(){
    ...
    while(reader.Next()){
        ...
        // if (*GATCONF_r == 32) {
            if(CATS->PositionOnTargetY > -200 && CATS->PositionOnTargetX > -200){
                Double_t targetX_byCATS = CATS->PositionOnTargetX;
                Double_t targetY_byCATS = CATS->PositionOnTargetY;


                hcats_recons_np4 -> Fill(targetX_byCATS,targetY_byCATS);
                
                if(CATS->DetNumber.size()==2){
                    if(CATS->DetNumber[0] == 1){
                        CATS1_X =  CATS->PositionX[0];
                        CATS1_Y =  CATS->PositionY[0];
                        CATS1_Z =  CATS->PositionZ[0];
                    }
                    if(CATS->DetNumber[1] == 2){
                        CATS2_X =  CATS->PositionX[1]/*  + 3.3 */;
                        CATS2_Y =  CATS->PositionY[1];
                        CATS2_Z =  CATS->PositionZ[1];
                    }
                }
