...
void loadFILES() {
  ...
  tree->Add("../../data/analysed/525_Ozge.root");
...
void build_CATS_check_Ozge(){
    ...
    TH2F *h_new = new TH2F("h_new", "CATS Target All Holes", 2400, -40, 40, 2400, -40, 40);
        ...
        if (*GATCONF_r == 4) {
            if(phy_cats_r->PositionOnTargetY > -200 && phy_cats_r->PositionOnTargetX > -200){
              ...
              if(phy_zdd_r->PL_E.size()>0){
                ...
                h_new->Fill(targetX_byCATS-1.7, targetY_byCATS-2.3);
...
/* 
    ...
    TCanvas *ozge = new TCanvas("ozge", "Trajectory Reconstruction", 1600, 800);
    h_new->SetTitle("CATS Target All Holes");
    h_new->GetXaxis()->SetTitle("X Position (mm)");
    h_new->GetYaxis()->SetTitle("Y Position (mm)");
    h_new->Draw("COLZ");
    target_mask();
/* 
