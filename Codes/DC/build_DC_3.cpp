 ...
 * 
 * I leave the build_DC_3 with the unified planes, next step is just to make
 * the spot with DC hit.
 *                                                                           *
 *****************************************************************************/
        ...
        if (*GATCONF_r == 1 || *GATCONF_r == 2 ||*GATCONF_r == 16 ||*GATCONF_r == 32) {
            if(phy_cats_r->PositionOnTargetY > -200 && phy_cats_r->PositionOnTargetX > -200){
              ...
              if(phy_zdd_r->PL_E.size()>0){
                ...
                if(TS_sub1>(-165) || TS_sub2>(-170)){   //excluding noise
                ...
                if(cut_right->IsInside(CATS1_X + ( Target_Z - Position[0])/ax,CATS1_Y + (Target_Z - Position[0])/ay) ){
                  ...
                  //the 4 quadrants
                  // if(dc3_flag && dc2_flag){dc_spot->Fill(TS_sub3_adj*DC_coeff[][],TS_sub2_adj)}  //quad 1
                  // if(dc4_flag && dc2_flag){dc_spot->Fill(}  //quad 2
