...
void loadFILES() {
  ...
  tree->Add("/home/sharmap/Workplace/np4/e843/data/analysed/525.root");
}
...
void implementing_DC(){
    ...
    while(reader.Next() /* && event_counter < 3000000 */) {
        ...
        if (/* *GATCONF_r == 1 || *GATCONF_r == 2 ||*GATCONF_r == 16 || */*GATCONF_r == 4) {
            if(phy_cats_r->PositionOnTargetY > -200 && phy_cats_r->PositionOnTargetX > -200){
              ...
              if(phy_zdd_r->PL_E.size()>0){
                ...
                unsigned long long pl_ts = (unsigned long long)(*GATCONFTS_r);
                ...
                if(DC_TS1_r.GetSize()==1){
                    ...
                    TS_sub1 = static_cast<Long64_t>(pl_ts) - static_cast<Long64_t>(dc_1_ts);
                    h1->Fill(TS_sub1);
                ...
                if(DC_TS2_r.GetSize()==1){
                    ...
                    TS_sub2 =  static_cast<Long64_t>(pl_ts) - static_cast<Long64_t>(dc_2_ts);
                    h2->Fill(TS_sub2);
                ...
                if(DC_TS3_r.GetSize()==1){
                    ...
                    TS_sub3 = static_cast<Long64_t>(pl_ts) - static_cast<Long64_t>(dc_3_ts);
                    h3->Fill(TS_sub3);
                ...
                if(DC_TS4_r.GetSize()==1){
                    ...
                    TS_sub4 =  static_cast<Long64_t>(pl_ts) - static_cast<Long64_t>(dc_4_ts);
                    h4->Fill(TS_sub4);
    ...
    // while(reader2.Next() /* && event_counter < 6000000 */) {
    //     event_counter++;

    //     if(event_counter % 1000000 == 0) {
    //         cout << "Processing entry (uncalibrated): " << (static_cast<double>(event_counter) / tree2->GetEntries()) * 100.0 << "%" << endl;}
    //     if(event_counter>=0 ){
