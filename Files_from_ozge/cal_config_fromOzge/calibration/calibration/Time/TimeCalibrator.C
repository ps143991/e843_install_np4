...
Double_t pol2 (Double_t *,Double_t *);
	Double_t pol1 (Double_t *,Double_t *);	
ofstream peaks_file, calib_file, dispersion_file,calib_online_file; 
	
void TimeCalibrator(const char* frun = "run_XXXX", TString xy = "X",Int_t npeak= 27,string fDet="MUST2", int TELESCOPE_START=1, int TELESCOPE_END=4)//49
//void TimeCalibrator(const char* frun = "r0106", TString xy = "Y",Int_t npeak= 24,string fDet="Mugast", int TELESCOPE_START=5, int TELESCOPE_END=5)//49
{
	...
	for (int det = TELESCOPE_START-1; det < TELESCOPE_END ; det++){
      ...
      if (fDet=="MUST2"){
		...
		for (Int_t j = STRIP_START-1; j < STRIP_END; j++)
		{
			...
			TimeGene(hist,xy,npeak, fDet, det);
...
void TimeGene(TH1F *hist, TString xy, Int_t npeak, string fDet, int DetNbr)
 ...
 // cout<<"Nbins after "<<Nbins<<endl;
  for(int i =0; i<Nbins; i++){
      if(hist_clone->GetBinCenter(i) > 8000 ) hist_clone->SetBinContent(i,0);
     // if (DetNbr!=10){//range change for the anular telescope 11
      if(hist_clone->GetBinCenter(i) < 3300 ) hist_clone->SetBinContent(i,0);
     // }
     // else {
...
/*
  ...
  */
  ...
  if(fDet=="MUST2"){
    ...
    //  calib_file << fDet << "_T" << CurrentTelescope << "_Si_X" << CurrentStrip << "_T " << a << " " << b <<endl;
    else if (xy == "Y")
      calib_file << fDet << "_T" << CurrentTelescope << "_Si_Y" << CurrentStrip << "_T " << a << " " << b << " " << c << endl ;
      //calib_file << fDet << "_T" << CurrentTelescope << "_Si_Y" << CurrentStrip << "_T " << a << " " << b <<endl;
  }
  else if(fDet=="Mugast"){
    if (xy == "X")
      calib_file << fDet << "_T" << CurrentTelescope << "_DSSD_X" << CurrentStrip << "_T " << a << " " << b << "	" << c <<endl ;
    //  calib_file << fDet << "_T" << CurrentTelescope << "_DSSD_X" << CurrentStrip << "_T " << a << " " << b <<endl ;

    else if (xy == "Y")
      calib_file << fDet << "_T" << CurrentTelescope << "_DSSD_Y" << CurrentStrip << "_T " << a << " " << b << " " << c << endl ;
      //calib_file << fDet << "_T" << CurrentTelescope << "_DSSD_Y" << CurrentStrip << "_T " << a << " " << b <<endl;
  }
