...
void TMugastPhysics::ReadAnalysisConfig() {
    bool ReadingStatus = false;
     ...
     // path to file
        string FileName = "./configs/ConfigMugast.dat";
        ...
        if (!AnalysisConfigFile.is_open()) {
          cout << " No ConfigMugast.dat found: Default parameters loaded for "
              "Analysis "
           << FileName << endl;
      return;
    }
    cout << " Loading user parameters for Analysis from ConfigMugast.dat " << endl;

    // Save it in a TAsciiFile
    TAsciiFile* asciiConfig = RootOutput::getInstance()->GetAsciiFileAnalysisConfig();
    asciiConfig->AppendLine("%%% ConfigMugast.dat %%%");
    ...
    while (!AnalysisConfigFile.eof()) {
      ...
      // search for "header"
      if (LineBuffer.compare(0, 11, "ConfigMugast") == 0)
        ReadingStatus = true;
  ...
  //for (unsigned int i = 0; i < blocks.size(); i++) {
        ...
        else if (whatToDo == "STRIP_ENERGY_MATCHING") {
            AnalysisConfigFile >> DataBuffer;
            m_StripEnergyMatching = atof(DataBuffer.c_str());
            cout << "STRIP ENERGY MATCHING" << m_StripEnergyMatching << endl;
            }
        ...
        else if (whatToDo == "DSSD_X_E_RAW_THRESHOLD") {
          AnalysisConfigFile >> DataBuffer;
          m_DSSD_X_E_RAW_Threshold = atof(DataBuffer.c_str());
          cout << whatToDo << " " << m_DSSD_X_E_RAW_Threshold << endl;
        }
        else if (whatToDo == "DSSD_Y_E_RAW_THRESHOLD") {
          AnalysisConfigFile >> DataBuffer;
          m_DSSD_Y_E_RAW_Threshold = atof(DataBuffer.c_str());
          cout << whatToDo << " " << m_DSSD_Y_E_RAW_Threshold << endl;
        }

        else if (whatToDo == "DSSD_X_E_THRESHOLD") {
          AnalysisConfigFile >> DataBuffer;
           m_DSSD_X_E_Threshold = atof(DataBuffer.c_str());
          cout << whatToDo << " " <<  m_DSSD_X_E_Threshold << endl;
        }
        else if (whatToDo == "DSSD_Y_E_THRESHOLD") {
          AnalysisConfigFile >> DataBuffer;
          m_DSSD_Y_E_Threshold = atof(DataBuffer.c_str());
          cout << whatToDo << " " << m_DSSD_Y_E_Threshold << endl;
        }
        else {
          ReadingStatus = false;
            }
