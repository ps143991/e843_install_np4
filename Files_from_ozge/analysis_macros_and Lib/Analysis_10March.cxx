...
void Analysis::TreatMugast2(){
 // double Channel_Calib2 = -0.0256053268;     // Calibration factor ozge mod2
 // double  Time_0_Ref2 = 778.321973;          // gain new calibration mod2
 // double  D4_CATS1_L2 = 2639;           // Path length correct

  double Channel_Calib2 = 0.02554;     // Calibration factor ozge mod2
  //double  Time_0_Ref2 = 29712.2;          // gain new calibration mod2
  double  D4_CATS1_L2 = 2150;           // Path length correct
  double  Time_0_Ref2 = 27394;          // gain new calibration mod2
  ...
  for(unsigned int countMugast = 0 ; countMugast < MG->DSSD_E.size() ; countMugast++){
      ...
      if (TAC_TOF == true) {
          TOF2 = (Time_0_Ref2 - TAC_D4_CATS1nc) * Channel_Calib2 / D4_CATS1_L2 / 1e7;  // experiment

        // TOF2= TAC_D4_CATS1nc*Channel_Calib2+Time_0_Ref2;
        //   velocity2 = TOF2/D4_CATS1_L2/1e7;
          reaction->GetNucleus1()->SetTimeOfFlight(TOF2);
          reaction->SetBeamEnergy(reaction->GetNucleus1()->GetEnergy());
          reaction->initializePrecomputeVariable();
          OriginalBeamEnergy2 = reaction->GetBeamEnergy();
        //  std::cout << "OriginalBeamEnergy2 " << OriginalBeamEnergy2 << std::endl;
          if(OriginalBeamEnergy2>1000){

            //       std::cout << "EnerMGtof1 " << OriginalBeamEnergy2 << std::endl;
              OriginalBeamEnergy2 = HeavyMylar.Slow( OriginalBeamEnergy2 ,1.5*micrometer , 0);
           //         std::cout << "EnerMGtof2 " << OriginalBeamEnergy2<< std::endl;
              OriginalBeamEnergy2 = Beam_Target.Slow( OriginalBeamEnergy2 , TargetThickness*0.5, 0);

// std::cout << "OriginalBeamEnergy2 -2 " << OriginalBeamEnergy2 << std::endl;
