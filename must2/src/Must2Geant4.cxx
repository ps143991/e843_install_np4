...
// nptool::geant4
// #include "CalorimeterScorers.h"
// #include "DSSDScorers.h"
// #include "InteractionScorers.h"
// #include "MaterialManager.h"
// #include "NPOptionManager.h"
// #include "nptool::geant4DetectorFactory.h"
// // ROOT
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// MUST2Geant4 Specific Method
namespace must2 {
// Default Resolution
G4double ResoTimeMust = 0.2127; // = 500ps                 //   Unit is  ns/2.35
G4double ResoStrip
    = 0.022; // 0.0223 = 52keV of Resolution   //   Unit is MeV/2.35  14.861996
...
const G4double VacBox_PosZ
    = AluStripBack_PosZ + 0.5 * AluStripThickness + 0.5 * VacBoxThickness;
const G4double CsI_PosZ
    = VacBox_PosZ + 0.5 * VacBoxThickness + 0.5 * CsIThickness;

MUST2Geant4::MUST2Geant4() {
  m_Event = new must2::Must2Data();
  // m_Event = std::make_shared<TMust2Data>();
  InitializeMaterial();
  m_StripScorer = 0;
  m_SiLiScorer  = 0;
  m_CsIScorer   = 0;
  m_detector    = std::dynamic_pointer_cast<must2::Must2Detector>(
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void MUST2Geant4::VolumeMaker(G4int TelescopeNumber, G4ThreeVector MMpos,
  ...
  std::cout << "I ADD VOLUMES" << std::endl;
  ...
  if (wCsI) {
    m_MaterialMyl = nptool::geant4::MaterialManager::GetMaterialManager()
                        ->GetMaterialFromLibrary("Mylar");
    m_MaterialCsI = nptool::geant4::MaterialManager::GetMaterialManager()
                        ->GetMaterialFromLibrary("CsI");
    ...
    new G4PVPlacement(Rotation(180., 0., 0.), positionCristal1, logicCristal1,
                      Name + "_CsI_Cristal1", logicCsI, false,
                      6); // 1
    ...
    new G4PVPlacement(Rotation(180., 0., 0.), positionCristal3, logicCristal3,
                      Name + "_CsI_Cristal3", logicCsI, false,
                      5); // 3
    new G4PVPlacement(Rotation(180., 0., 0.), positionCristal4, logicCristal4,
                      Name + "_CsI_Cristal4", logicCsI, false,
                      4); // 4
    ...
    new G4PVPlacement(Rotation(180., 0., 0.), positionCristal2b, logicCristal2,
                      Name + "_CsI_Cristal6", logicCsI, false,
                      15); // 6
    ...
    new G4PVPlacement(Rotation(180, 0, 0), positionCristal2sb, logicCristal2s,
                      Name + "_CsI_Cristal14", logicCsI, false,
                      2); // 14
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// Virtual Method of nptool::geant4::VDetector class

//// Read stream at Configfile to pick-up parameters of detector (Position,...)
//// Called in DetecorConstruction::ReadDetextorConfiguration Method
// void MUST2Geant4::ReadConfiguration(NPL::InputParser parser) {
//   vector<NPL::InputBlock*> blocks =
//   parser.GetAllBlocksWithToken("M2Telescope"); if
...
//      G4ThreeVector A =
//      nptool::geant4::ConvertVector(blocks[i]->GetTVector3("X1_Y1", "mm"));
//      G4ThreeVector B =
//      nptool::geant4::ConvertVector(blocks[i]->GetTVector3("X128_Y1", "mm"));
//      G4ThreeVector C =
//      nptool::geant4::ConvertVector(blocks[i]->GetTVector3("X1_Y128", "mm"));
//      G4ThreeVector D =
//      nptool::geant4::ConvertVector(blocks[i]->GetTVector3("X128_Y128",
//      "mm")); int SI = blocks[i]->GetInt("SI"); int SILI =
//      blocks[i]->GetInt("SILI"); int CSI = blocks[i]->GetInt("CSI");
//      AddTelescope(A, B, C, D, SI == 1, SILI == 1, CSI == 1); m_CsIOffset[i +
//      1] = blocks[i]->GetInt("CsIOffset");
//    }
...
//      AddTelescope(R, Theta, Phi, beta[0], beta[1], beta[2], SI == 1, SILI ==
//      1, CSI == 1); m_CsIOffset[i + 1] = blocks[i]->GetInt("CsIOffset");
//    }
...
//    return;
//  }
//  cout << " Loading user parameters for Analysis from ConfigMust2.dat " <<
//  endl;
...
void MUST2Geant4::ConstructDetector(/*G4LogicalVolume* world*/) {
  std::cout << ">>>>>>>>>> DEBUG : " << 0 << std::endl;
  std::cout << "I CONSTRUCT" << std::endl;
  ...
  G4int NumberOfTelescope = m_detector->GetNumberOfTelescopes();

  for (G4int i = 0; i < NumberOfTelescope; i++) {
    // By Point
    // FIXME For now only corner based geometry is implemented
    if (true /*m_DefinitionType[i]*/) {
      // (u,v,w) unitary vector associated to telescope referencial
      // (u,v) // to silicon plan
      // w perpendicular to (u,v) plan and pointing CsI
      G4ThreeVector MMu(
          m_detector->m_X128_Y1[i].X() - m_detector->m_X1_Y1[i].X(),
          m_detector->m_X128_Y1[i].Y() - m_detector->m_X1_Y1[i].Y(),
          m_detector->m_X128_Y1[i].Z() - m_detector->m_X1_Y1[i].Z());
      MMu = MMu.unit();

      G4ThreeVector MMv(
          m_detector->m_X1_Y128[i].X() - m_detector->m_X1_Y1[i].X(),
          m_detector->m_X1_Y128[i].Y() - m_detector->m_X1_Y1[i].Y(),
          m_detector->m_X1_Y128[i].Z() - m_detector->m_X1_Y1[i].Z());
      MMv = MMv.unit();

      G4ThreeVector MMw = MMv.cross(MMu);
      // if (MMw.z() > 0)MMw = MMv.cross(MMu)  ;
      MMw = MMw.unit();

      G4ThreeVector MMCenter(
          (m_detector->m_X1_Y1[i].X() + m_detector->m_X1_Y128[i].X()
           + m_detector->m_X128_Y1[i].X() + m_detector->m_X128_Y128[i].X())
              / 4,
          (m_detector->m_X1_Y1[i].Y() + m_detector->m_X1_Y128[i].Y()
           + m_detector->m_X128_Y1[i].Y() + m_detector->m_X128_Y128[i].Y())
              / 4,
          (m_detector->m_X1_Y1[i].Z() + m_detector->m_X1_Y128[i].Z()
           + m_detector->m_X128_Y1[i].Z() + m_detector->m_X128_Y128[i].Z())
              / 4);
    ...
    Si   = m_detector->m_SiPresent[i + 1];
    SiLi = m_detector->m_SiLiPresent[i + 1];
    CsI  = m_detector->m_CsIPresent[i + 1];
...
// void MUST2Geant4::InitializeRootOutput() {
...
void MUST2Geant4::ReadSensitive(const G4Event*) {
  /*
     m_Event->Clear();
  ...
  double timeX
  = TimeOffset - RandGauss::shoot(it->second.second, ResoTimeMust);
  ...
  if (SiScoredHit) {
    ...
    for (unsigned int i = 0; i < sizeSiLi; i++) {
      ...
      if (ESiLi > ThresholdSiLi) {
        ...
        double timeSiLi
          = RandGauss::shoot(SiLiScorer->GetTime(i), ResoTimeMust);
      ...
      if (ECsI > ThresholdCsI) {
        if (m_CsIOffset[level[0]] == 1)
          m_Event->SetCsIE(level[0], level[1],
              NPL::EnergyToADC(ECsI, 0, 500, 0, 16384));
        else
          m_Event->SetCsIE(level[0], level[1],
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void MUST2Geant4::InitializeScorers() {
  /*
  ...
  nptool + "/NPLib/Detectors/MUST2/ressources/maskBack.png", 97.22 / 12800,
  97.22 / 12800, 0, 0, 0xffff0000, 0);
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void MUST2Geant4::InitializeMaterial() {
  m_MaterialSilicon = nptool::geant4::MaterialManager::GetMaterialManager()
                          ->GetMaterialFromLibrary("Si");
  m_MaterialAluminium = nptool::geant4::MaterialManager::GetMaterialManager()
                            ->GetMaterialFromLibrary("Al");
  m_MaterialIron = nptool::geant4::MaterialManager::GetMaterialManager()
                       ->GetMaterialFromLibrary("Fe");
  m_MaterialVacuum = nptool::geant4::MaterialManager::GetMaterialManager()
...
//////////////////////////////////////////////////////////////////////////////////
// nptool::geant4::VDetector* MUST2Geant4::Construct() {
//   return (nptool::geant4::VDetector*)new MUST2Geant4();
// }
...
std::shared_ptr<nptool::geant4::VDetector> ConstructDetectorSimulation() {
  return make_shared<must2::MUST2Geant4>();
};
}
