 ...
 *                                                                           *
 *---------------------------------------------------------------------------*
 * Comment: PS: to be used for e869 expt. changing several, for the e843 use
 * the file Analysis_working_new_e843.cxx
 *                                                                           *
 *****************************************************************************/
...
bool Analysis::UnallocateBeforeBuild(){
    ...
    //  if (GATCONFMASTER[0]==64)
    // 	{
    //         //cout<<"PS: gatconfmaster found: "<<GATCONFMASTER[0]<<endl;
    //     	//SetRefTS();
...
void Analysis::TreatEvent(){
    ...
    if(counterTAC>1)cout<<"tac RAN twice"<<endl;
    TreatCATS();
    //TreatTAC();
    //TreatMugast();
    TreatZDD();

        //std::cout<<"PS: treatevent() ran too"<<std::endl;
    ...
    for(unsigned int i = 0; i < TAC->TAC_Time.size(); i++){
        if(TAC->TAC_Name[i].compare("TAC_CATS_PL") == 0)
        {
           ...
           // cout<<"the incoming cats_pl value is: "<<TAC->TAC_Time[i]<<endl;
            counterTAC++;
            TAC_CATS_PL += (TAC->TAC_Time[i]*0.01276894);
            TAC_CATS_PL_TS.push_back(TAC->TAC_TS[i]);
            if (TAC_CATS_PL>0) counterPS4++;
        }
...
void Analysis::TreatZDD(){
    ...
    for (int i=0; i<IC_M;i++){
        //cout<<"IC_E_ "<<ZDD->IC_Nbr[i]<<" energy "<<ZDD->IC_E[i]<<endl;
        if(ZDD->IC_Nbr[i]==0)      {IC_1.push_back(ZDD->IC_E[i]);   if(TAC_CATS_PL>0 && ZDD->IC_E[i]>0)counterPS1++;}
        else if(ZDD->IC_Nbr[i]==1) {IC_2.push_back(ZDD->IC_E[i]);   if(TAC_CATS_PL>0 && ZDD->IC_E[i]>0)counterPS2++;}
        else if(ZDD->IC_Nbr[i]==2) {IC_3.push_back(ZDD->IC_E[i]);   if(TAC_CATS_PL>0 && ZDD->IC_E[i]>0)counterPS3++;}
...
void Analysis::End(){
    ...
    Trajectory_YZ->Write();
    PL_spot_const->Write();
    cout<<"IC1 (with IC1>0 && TAC_CATS_PL>0) is: "<<counterPS1<<endl; 
    cout<<"IC2 (with IC2>0 && TAC_CATS_PL>0) is: "<<counterPS2<<endl; 
    cout<<"IC3 (with IC3>0 && TAC_CATS_PL>0) is: "<<counterPS3<<endl; 
    cout<<"TAC_CATS_PL (with TAC_CATS_PL>0) is: "<<counterPS4<<endl; 
}
