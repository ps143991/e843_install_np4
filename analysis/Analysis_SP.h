...
namespace user_analysis {
  class Analysis : public nptool::VUserAnalysis {
   ...
   private:
    ...
    std::pair<double, double> tac_D4_CATS1;
    std::pair<double, double> tac_CATS_PL;
    std::pair<double, double> tac_MMG_CATS1;
    std::pair<double, double> tac_MMG_EXOGAM;
    std::pair<double, double> tac_CATS_HF;
    std::pair<double, double> tac_CATSD6_1_DD4;
    std::pair<double, double> tac_PL_2;
    std::pair<double, double> tac_PL_4;
    ...
    double TOF;
    double TOFLength;
    const double D4_CATS1_L = 21500; // Distance between D4 and CATS1 in mm
    const double Channel_Calib = 0.02554;    // Time Calibration Coefficient UPDATED 17 Dec 2025 
    //const double Channel_Calib = 0.00247;    // Time Calibration Coefficient UPDATED 17 Dec 2025
    // original 0.02554 ns?/mm channel
    // const double Channel_Calib = 0.00250;    // Time Calibration Coefficient  for (p,d)
    const double Time_0_Ref = 27594;  // T0 reference for TOF calculation
    const double mylar_thickness = 8.4; // in micrometer
    const double al_thicknessMG = 0.4; // in micrometer
    ...
    bool bMUST2 = false;
    unsigned long long m_total_event = 0;
    unsigned long long m_kept_event = 0;
...
/*     Reaction S36pdReaction;
    ...
    nptool::Reaction* reaction_must2;


    EnergyLoss* Beam_Target;
    EnergyLoss* HeavyTarget;
    EnergyLoss* LightTarget;
    EnergyLoss* LightAl;
    EnergyLoss* LightSi;
    EnergyLoss* HeavyMylar;
